...
class AndroidTtsSpeaker(context: Context) : Speaker {
    ...
    private val localeByLanguageCode = mapOf(
        "en" to Locale("en"),
        "es" to Locale("es"),
        "de" to Locale("de"),
    )
    ...
    private fun speakNow(text: String, languageCode: String) {
        ...
        if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
            Log.w(TAG, "[event=tts.locale_unavailable method=speak] locale unavailable, skipping pronunciation | languageCode=$languageCode")
            return
        }
