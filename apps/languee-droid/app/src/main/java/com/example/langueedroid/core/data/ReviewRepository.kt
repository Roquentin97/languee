...
class ReviewRepository(
    ...
    suspend fun summary(): Result<ReviewSummary> = runCatching {
        ...
        when {
            response.isSuccessful -> {
                ...
                Log.i(TAG, "[event=review.summary_loaded method=summary] summary loaded | dueCount=${body.dueCount} newCount=${body.newCount}")
                body
            }
    ...
    suspend fun queue(deckId: String? = null, limit: Int = DEFAULT_QUEUE_LIMIT): Result<List<ReviewItem>> = runCatching {
    ...
    suspend fun checkAnswer(cardId: String, typedAnswer: String): Result<AnswerCheck> = runCatching {
        val response = reviewsApi.checkAnswer(
            cardId = cardId,
    ...
    suspend fun grade(
    ...
    ): Result<GradeOutcome> = runCatching {
        val response = reviewsApi.grade(
            cardId = cardId,
            body = GradeRequest(
        ...
        when {
            response.isSuccessful -> {
                ...
                Log.i(TAG, "[event=review.card_graded method=grade] card graded | cardId=$cardId rating=$rating state=${body.state} intervalDays=${body.intervalDays}")
                body
            }
            response.code() == 401 -> throw UnauthorizedException()
            response.code() == 404 -> throw StaleReferenceException()
    ...
    private fun ReviewRating.toWireValue(): String = when (this) {
