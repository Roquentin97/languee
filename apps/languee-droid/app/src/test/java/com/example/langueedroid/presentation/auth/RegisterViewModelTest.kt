...
import com.example.langueedroid.feature.auth.presentation.RegisterViewModel
...
class RegisterViewModelTest {
    ...
    @Test
    fun `onRegisterClick with blank email shows Error state`() =
        runTest {
            val vm = buildViewModel()
            vm.onEmailChange("   ")
    ...
    @Test
    fun `onRegisterClick with blank password shows Error state`() =
        runTest {
            val vm = buildViewModel()
            vm.onEmailChange("user@example.com")
            vm.onPasswordChange("")
    ...
    @Test
    fun `onRegisterClick with blank confirmPassword shows Error state`() =
        runTest {
            val vm = buildViewModel()
            vm.onEmailChange("user@example.com")
            vm.onPasswordChange("secret")
    ...
    @Test
    fun `onRegisterClick with blank fields never calls authRepository`() =
        runTest {
            val vm = buildViewModel()
            vm.onEmailChange("")
            vm.onPasswordChange("")
    ...
    @Test
    fun `onRegisterClick with mismatched passwords shows Error without network call`() =
        runTest {
            val vm = buildViewModel()
            vm.onEmailChange("user@example.com")
    ...
    @Test
    fun `onRegisterClick success sets Success state with session`() =
        runTest {
            val vm = buildViewModel()
    ...
    @Test
    fun `onRegisterClick success transitions through Loading then resolves to Success`() =
        runTest {
            val vm = buildViewModel()
            val session = fakeSession()
    ...
    @Test
    fun `onRegisterClick network exception shows Error state`() =
        runTest {
            val vm = buildViewModel()
    ...
    @Test
    fun `onRegisterClick network exception leaves state as Error not Success`() =
        runTest {
            val vm = buildViewModel()
            whenever(authRepository.register(any(), any()))
                .thenReturn(Result.failure(RuntimeException("Network error")))
    ...
    @Test
    fun `onRegisterClick repository failure shows Error state`() =
        runTest {
            val vm = buildViewModel()
            whenever(authRepository.register(any(), any()))
    ...
    @Test
    fun `onRegisterClick sets Loading state then resolves after register call`() =
        runTest {
            val vm = buildViewModel()
            val gate = Channel<Unit>()
            whenever(authRepository.register(any(), any())).doSuspendableAnswer {
    ...
    @Test
    fun `second onRegisterClick while Loading is ignored`() =
        runTest {
            val vm = buildViewModel()
            val gate = Channel<Unit>()
            var callCount = 0
    ...
    @Test
    fun `onRegisterClick with null exception message shows fallback error`() =
        runTest {
            val vm = buildViewModel()
            whenever(authRepository.register(any(), any()))
