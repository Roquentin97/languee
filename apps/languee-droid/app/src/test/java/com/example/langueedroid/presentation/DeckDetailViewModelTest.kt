...
class DeckDetailViewModelTest {
    ...
    private fun buildViewModel(deckId: String = "d1") = DeckDetailViewModel(
        deckId = deckId,
        cardRepository = cardRepository,
    )

    private fun aCard(id: String = "c1") = Card(
        id = id,
        deckId = "d1",
        lemma = "cat",
        partOfSpeech = "noun",
    ...
    @Test
    fun `cards load on entry — Loaded state with the deck cards`() = runTest {
    ...
    @Test
    fun `deck with no cards — Empty state`() = runTest {
    ...
    @Test
    fun `load failure — Error state`() = runTest {
    ...
    @Test
    fun `retry after failure reloads the cards`() = runTest {
        whenever(cardRepository.listCards("d1")).thenReturn(
            Result.failure(RuntimeException("HTTP 500")),
        )
        val vm = buildViewModel()
    ...
    @Test
    fun `401 — unauthorizedEvent is emitted`() = runTest {
