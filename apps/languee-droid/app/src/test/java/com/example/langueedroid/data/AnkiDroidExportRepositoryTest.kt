...
class AnkiDroidExportRepositoryTest {
    ...
    @Test
    fun `createOrGetExportRecord 200 returns domain AnkiDroidExport with pending status`() =
        runTest {
    ...
    @Test
    fun `createOrGetExportRecord maps completed status correctly`() =
        runTest {
            val dto =
                anExportDto(
                    status = "completed",
    ...
    @Test
    fun `createOrGetExportRecord maps failed status with reason and message`() =
        runTest {
            val dto =
                anExportDto(
                    status = "failed",
    ...
    @Test
    fun `createOrGetExportRecord 401 throws UnauthorizedException`() =
        runTest {
            whenever(api.createOrGetExport(any())).thenReturn(
    ...
    @Test
    fun `createOrGetExportRecord 404 throws StaleReferenceException`() =
        runTest {
            whenever(api.createOrGetExport(any())).thenReturn(
    ...
    @Test
    fun `createOrGetExportRecord 500 returns generic failure`() =
        runTest {
            whenever(api.createOrGetExport(any())).thenReturn(
    ...
    @Test
    fun `createOrGetExportRecord null body returns failure`() =
        runTest {
    ...
    @Test
    fun `recordAttemptCompleted 200 returns success`() =
        runTest {
    ...
    @Test
    fun `recordAttemptCompleted 401 throws UnauthorizedException`() =
        runTest {
    ...
    @Test
    fun `recordAttemptFailed 200 returns success`() =
        runTest {
            whenever(api.recordAttempt(eq("export-1"), any())).thenReturn(
                Response.success(Unit),
            )
            ...
            val result =
                repository.recordAttemptFailed(
                    exportId = "export-1",
                    failureReason = "permission_not_granted",
                    failureMessage = "AnkiDroid permission denied",
                )
    ...
    @Test
    fun `recordAttemptFailed 401 throws UnauthorizedException`() =
        runTest {
            whenever(api.recordAttempt(any(), any())).thenReturn(
                Response.error(401, "{}".toResponseBody()),
            )
    ...
    @Test
    fun `recordAttemptFailed 500 returns generic failure`() =
        runTest {
            whenever(api.recordAttempt(any(), any())).thenReturn(
                Response.error(500, "{}".toResponseBody()),
            )
    ...
    @Test
    fun `getCardsWithPendingExport merges pending and failed card ids`() =
        runTest {
    ...
    @Test
    fun `getCardsWithPendingExport deduplicates ids appearing in both pending and failed`() =
        runTest {
            whenever(api.getCardsWithExportStatus(eq("pending"), anyOrNull())).thenReturn(
    ...
    @Test
    fun `getCardsWithPendingExport returns empty list when both responses are empty`() =
        runTest {
            whenever(api.getCardsWithExportStatus(eq("pending"), anyOrNull())).thenReturn(
    ...
    @Test
    fun `getCardsWithPendingExport 401 on pending query throws UnauthorizedException`() =
        runTest {
            whenever(api.getCardsWithExportStatus(eq("pending"), anyOrNull())).thenReturn(
                Response.error(401, "{}".toResponseBody()),
            )
    ...
    @Test
    fun `getCardsWithPendingExport 401 on failed query throws UnauthorizedException`() =
        runTest {
            whenever(api.getCardsWithExportStatus(eq("pending"), anyOrNull())).thenReturn(
                Response.success(emptyList()),
            )
    ...
    @Test
    fun `getCardsWithPendingExport 500 on failed query returns generic failure`() =
        runTest {
            whenever(api.getCardsWithExportStatus(eq("pending"), anyOrNull())).thenReturn(
                Response.success(emptyList()),
            )
