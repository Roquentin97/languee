...
class AppSessionViewModelAnkiTest {
    ...
    private fun incompleteSetupPrefs() =
        AnkiDroidSetupPrefs(
    ...
    @Test
    fun `onAuthSuccess routes to AuthorizedPendingAnkiSetup when setup not completed`() =
        runTest {
    ...
    @Test
    fun `onAuthSuccess routes to Authorized when setup already completed`() =
        runTest {
            whenever(sessionStore.read()).thenReturn(null)
    ...
    @Test
    fun `startup refresh routes to AuthorizedPendingAnkiSetup when setup not completed`() =
        runTest {
            val stored = aSession()
    ...
    @Test
    fun `startup refresh routes to Authorized when setup already completed`() =
        runTest {
            val stored = aSession()
            whenever(sessionStore.read()).thenReturn(stored)
            whenever(authRepository.refreshSession()).thenReturn(Result.success(stored))
    ...
    @Test
    fun `onAnkiSetupFinished from AuthorizedPendingAnkiSetup transitions to Authorized`() =
        runTest {
            whenever(sessionStore.read()).thenReturn(null)
            whenever(prefsStore.read()).thenReturn(incompleteSetupPrefs())
    ...
    @Test
    fun `onAnkiSetupFinished saves prefs with setupCompleted true`() =
        runTest {
            whenever(sessionStore.read()).thenReturn(null)
            whenever(prefsStore.read()).thenReturn(incompleteSetupPrefs())
    ...
    @Test
    fun `onAnkiSetupFinished from Authorized state stays Authorized`() =
        runTest {
            whenever(sessionStore.read()).thenReturn(null)
            whenever(prefsStore.read()).thenReturn(completedSetupPrefs())
    ...
    @Test
    fun `onAnkiSetupFinished from Unauthorized state does nothing`() =
        runTest {
            whenever(sessionStore.read()).thenReturn(null)
            whenever(prefsStore.read()).thenReturn(incompleteSetupPrefs())
