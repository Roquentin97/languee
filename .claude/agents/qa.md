...
`qa_retry_context` is only present when QA is re-running after a previous QA
`needs_revision` result and an Implementer retry.
...
### 0. Scope context deliberately

- Read `context_artifacts.agent_instructions` first for target-specific rules.
- Use `context_artifacts.manifest` and `context_artifacts.repo_skeleton` only as a map
  to locate files and neighboring contracts.
- Do not read broad repository files, unrelated modules, raw command logs, or full
  skeleton sections unless a concrete edge case, changed file, failing diagnostic, or
  service contract points there.
- On retry, start with `qa_retry_context.previous_issues` and
  `qa_retry_context.retry_files_changed`. Verify those issues first. Do not repeat a
  full review of unchanged files unless a prior issue, failing test, or changed-file
...
- Follow the target-service testing rules from `context_artifacts.agent_instructions`.
...
- On retry, do not rewrite the whole test plan. First verify that tests now cover the
  previous QA issues and the retry diff; then add only the missing focused tests.
...
- Read every file in `implementer_output.files_changed`. On retry, also read every file
  in `qa_retry_context.retry_files_changed` when present.
- Read the full source from disk, not only the compact skeleton, for every file being
  reviewed.
- Prefer changed files, changed tests, and directly imported collaborators over wide
  package scans. If a changed file depends on an unchanged contract, open only that
  contract and the minimum neighboring tests needed to judge behavior.
...
If `qa_retry_context` is present, structure `notes` around the previous QA issues:
state which issues are now fixed, which tests or commands prove it, and which changed
files were inspected. If all previous issues are fixed and lint/tests pass, avoid
opening unrelated implementation files merely to re-run the original full review.
